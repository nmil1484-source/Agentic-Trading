...
- Reward-to-risk must be at least 1:2 (distance to target ≥ 2x distance to stop). If the math
  ...
  UNKNOWN_DEGRADED (§6, added 2026-08-13), this floor rises to ≥1:3.** **Fractional Tier-B pilots
  (§15) use their own separate floor instead of this one — ≥1.5:1 normally, ≥2:1 while
  UNKNOWN_DEGRADED** — this 1:2/1:3 floor is unchanged for standard (Tier-A) proposals.
...
Added 2026-08-13 at explicit user instruction. Full text also maintained in
`docs/fractional_tier_b_policy.md` for standalone reference; if the two ever diverge, this
section in CLAUDE.md is authoritative. For clarity, the existing whole-share process (§§1-14) is
referred to as **Tier-A** here; this new fractional-share lane is **Tier-B**. Tier-B is a scoped,
tightly-capped exception carved out of the existing rules, not an additional or parallel
allowance — it counts inside the existing §3 position-count and total-deployment limits (see item
7), not on top of them, and does not loosen any Tier-A rule that isn't explicitly modified below.
Every Tier-A rule not explicitly modified below still applies in full to Tier-B: no
...
3. A Tier-B pilot is eligible when LUC is GREEN or WHITE. LUC RED is always no-entry. This is a
   scoped exception to §2(a)'s GREEN-only requirement, available only to Tier-B and only subject
   to the stricter confirmation in item 4 below when LUC is WHITE.
...
5. Require a verified catalyst or sector tailwind (URL and date, per §5.4). Minimum reward-to-
   ...
   instead** — stronger than Tier-B's normal 1.5:1, though still looser than Tier-A's 3:1-while-
   degraded floor from §13.A — combined with the item 2 halved allocation cap while in that state.
   This is a scoped exception to §13.A's floor, which is unchanged for Tier-A. Do not require a
   perfect FTA A-grade score for a Tier-B pilot — LUC status plus the item 4 confirmations (when
...
## 16. Locked Exit and Loss-Control Policy (Tier-B Fractional Pilots)
Added 2026-08-13 at explicit user instruction. Applies to every Tier-B fractional pilot (§15).
Full text also maintained in `docs/fractional_tier_b_policy.md`; CLAUDE.md is authoritative if
the two diverge. All existing circuit breakers (§6) remain in force unchanged and
...
3. Stop execution: when last price trades at or below the documented invalidation, immediately
   ...
   remaining fractional position. Do not widen, remove, or lower the stop. If the exit cannot be
   verified or submitted, create an URGENT EXIT FAILURE alert and halt all new Tier-B entries
   (Tier-A is unaffected unless a §6 circuit breaker independently triggers).
...
9. No averaging down. No exception for "oversold," social-media sentiment, or a lower price. An
   invalidated Tier-B trade is closed, documented, and not re-entered for 30 calendar days unless
   a fresh setup meeting the full Tier-A bar (§2/§5/§13, not just Tier-B's lighter bar) forms.
